document.addEventListener('DOMContentLoaded', () => {
    ...
    // Generate Dots (1 dot per 4 cars)
    const dealersDotsContainer = document.getElementById('dealersDots');
    let dealersDots = [];
    if (dealersDotsContainer) {
        const numDots = Math.ceil(dealerPanels.length / 4);
        for (let i = 0; i < numDots; i++) {
            const dot = document.createElement('span');
            dot.className = 'dealer-dot';
            // highlight logic
            if (i === Math.floor(currentIndex / 4)) dot.classList.add('active');
            dot.addEventListener('click', () => {
                if (typeof isHoverRotating !== 'undefined' && isHoverRotating) return;
                const targetIndex = i * 4;
                if (currentIndex === targetIndex) return;
                if (typeof autoRotate !== 'undefined') clearInterval(autoRotate);
                currentIndex = targetIndex;
                if (typeof triggerAnimation === 'function') triggerAnimation();
            });
            dealersDotsContainer.appendChild(dot);
            dealersDots.push(dot);
        }
    }
    ...
    const updateClasses = () => {
        ...
        const halfHidden = Math.floor((total - 4) / 2);
        const base = currentIndex - halfHidden;
        ...
        dealerPanels.forEach((panel, i) => {
            ...
            // Maintain stable order for visible items to prevent CSS flex transition jank
            panel.style.order = base + safeMod(i - base, total);
        ...
        if (dealersDots.length) {
            dealersDots.forEach((dot, i) => {
                dot.classList.toggle('active', i === Math.floor(currentIndex / 4));
            });
        }
    };
